...
@Component({
    selector: 'io-multiple-answer-form',
    templateUrl: './multiple-answer-form.component.html',
    styleUrls: ['./multiple-answer-form.component.scss'],
})
export class MultipleAnswerFormComponent implements OnInit {
    /** Form data */
    ...
    submit() {
        ...
        if (this.areValidAnswers()) {
            ...
            this.toastrWrapper.warning(
                'questions.messages.not_valid_answers_multiple.message',
                'common.warning'
            );
        }
    }
    ...
    private areValidAnswers(): boolean {
        return (
            this.question.answers.filter(q => !Utils.isStringValid(q.text)).length === 0 &&
            this.question.answers.filter(q => q.correct === true).length > 0
        );
    }
    ...
    addAnswer() {
        if (this.question.answers.length === 4) {
            ...
            let live = this.elementRef.nativeElement.querySelector('#multiple-answer-answers-live');
            setTimeout(
                () =>
